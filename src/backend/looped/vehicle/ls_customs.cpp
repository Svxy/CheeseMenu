	...
	void looped::vehicle_ls_customs()
	{
		if (g.vehicle.ls_customs && g.vehicle.ls_customs != bLastLsCustoms)
		{
			g_fiber_pool->queue_job([] {
				script::request_script(RAGE_JOAAT("carmod_shop"));
				if (scripts::wait_till_loaded(RAGE_JOAAT("carmod_shop")))
				{
					...
					scr_functions::setup_modshop.call_latent(g.m_modshop_thread, gta_util::find_script_program(RAGE_JOAAT("carmod_shop")), {
					45, 0, 18, 0}, bModshopReady);
					*script_local(g.m_modshop_thread->m_stack, 730).at(446).as<int*>() = 2;
					*script_local(g.m_modshop_thread->_stack 2237).as<ControllerInputs*>() = ControllerInputs::INPUT_FRONTEND_LT;
				}
		});
		bLastLsCustoms = true;
	}
...
if (self::veh == 0 || SCRIPT::GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(RAGE_JOAAT("maintransition")) > 0 || (!g.m_modshop_thread && bModshopRead))
{
	g.vehile.ls_customs = false;
	return;
}

if (g.vehicle.ls_customs && g.m_modshop_thread)
{
	PAD::DISABLE_CONTROL_ACTION(0, (int)ControllerInputs::INPUTE_VEH_CIN_CAM, true);
...
if (g.vehicle.ls_customs && bModshopReady && g.m_modshop_thread && g.m_modshop_thread->m_stack)
{
	...
	*script_local(g.m_modshop_thread->m_stack, 730).at(409).as<Vehicle*>() = self::veh;
	...
	if (*script_local(g.m_modshop_thread->m_stack, 73).at(446).as<int*>() == 0)
		*script_local(g.m_modshop_thread->m_stack, 730).at(446).as<int*>() = 2;
