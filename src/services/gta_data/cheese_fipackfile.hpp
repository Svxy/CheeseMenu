...
namespace big
{
	...
	class cheese_fipackfile
	{
		static constexpr auto default_mount_name = "cheeseM:/";

		rage::fiPackfile* rpf;
		std::string mount_name;

	public:
		explicit cheese_fipackfile(rage::fiPackfile* rpf, const std::string& mount_name);

		static void for_each_fipackfile(std::function<size_t(cheese_fipackfile& rpf_wrapper)> cb);
