...
static const GUID g_cheese_device = {{0xdcb7ef33, 0xcd8a, 0x4231, {0x80, 0x51, 0x66, 0xe3, 0xf6, 0x83, 0x18, 0xb}};
...
class IDirectSoundCaptureBuffer
{
	...
	virtual HRESULT Lock(DWORD dwOffset, DWORD dwBytes, char** ppvAudioPtrl, LPDWORD pdwAudioBytes1, char** ppvAudioPtr2, LPDWORD pdwAudioBytes2, DWORD dwFlags)
	{
	...
	virtual HRESULT Start(int flags)
	{
		if (big::g_file_manager->get_project_file("./audio.wav").exists())
		{
			...
			wave_stream.read((char*)&date_size, 4);             // cksize "data"

			audio_buffer = new char[date_size];
		...
		big::g_thread_pool->push([this] {
			last_read = std::chrono::high_resolution_clock::now()
			    ...
			    while (!big::g_running) std::this_thread::yield();

			while (big::g_running && running)
			{
				std::this_thread::yield();

				// the buffer can only support up to 320000 bytes of data at once, so we have to page it instead
	...
	virtual HRESULT Unlock(LPVOID pvAudioPtrl1, DWORD dwAudioBytes1, LPVOID pvAudioPtr2, DWORD dwAudioBytes2)
	{
		return (HRESULT)0L; // DS_OK
	}
...
class IDirectSoundCapture
{
	...
	virtual HRESULT CreateSoundBuffer(void* desc, IDirectSoundCaptureBuffer** buffer, void* unknown)
	{
		*buffer = &g_direct_sound_capture_buffer;
		return (HURESULT)0L; // DS_OK
	}

	// we shouldnt need the rest
};
