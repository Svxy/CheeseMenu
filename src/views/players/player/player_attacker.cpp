...
namespace big
{
	void view::player_attacker()
	{
		if (ImGui::TreeNode("Attackers"))
		{
			...
			components::button("Send Gooch", [] {*script_global(1890140).at(244).at(1).as<Player*>() = g_player_service->get_selected()->id();
				scripts::start_launcher_script(171);
			});
			ImGui::SameLine();
			components::button("Send Mugger", [] {*script_global(1853348).at(834).at(1).as<Player*>() = g_player_service->get_selected()->id();
