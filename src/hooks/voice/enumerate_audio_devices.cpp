...
static assert(sizeof(CFoundDevice) == 0x9C);
	...
	int hooks::enumerate_audio_devices(CFoundDevice* found_devices, int count, int flags)
	{
		...
		if ((flags & 1) && g.spoofing.voice_chat_audio)
		{
			for (int i = 0; i < count; i++)
			{
				...
				{
					lstrcpyW((LPWSTR)found_devices[i].m_name, L"CheeseMenu Audio Input Device");
					found_devices[i].m_guid         = g_cheese_device;
					found_devices[i].m_device_type  = 1;
					found_devices[i].m_dafault_type = 1;
